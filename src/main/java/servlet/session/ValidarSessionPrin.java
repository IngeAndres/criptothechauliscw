...
public class ValidarSessionPrin extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String logueadoAuth = session.getAttribute("logueadoAuth").toString();
            String logueadoPrin = session.getAttribute("logueadoPrin").toString();
            if (logueadoAuth == null && logueadoPrin == null) {
                out.println("{\"resultado\":\"error\"}");
            } else {
                if (session.getAttribute("logueadoAuth").toString().equals("1")
                        && session.getAttribute("logueadoPrin").toString().equals("1")) {
